...
kube_istio_source_packages = $(subst $(space),$(empty), \
	github.com/aeraki-mesh/api/dubbo/v1alpha1, \
	github.com/aeraki-mesh/api/redis/v1alpha1, \
	github.com/aeraki-mesh/api/metaprotocol/v1alpha1 \
	)

# base output package for generated files
kube_base_output_package = github.com/aeraki-mesh/client-go/pkg
...
kube_api_packages = $(subst $(space),$(empty), \
	$(kube_api_base_package)/dubbo/v1alpha1, \
	$(kube_api_base_package)/redis/v1alpha1, \
	$(kube_api_base_package)/metaprotocol/v1alpha1 \
	)
...
rename_generated_files=\
	find $(subst github.com/aeraki-mesh/client-go/, $(empty), $(subst $(comma), $(space), $(kube_api_packages)) $(kube_clientset_package) $(kube_listers_package) $(kube_informers_package)) \
	-name '*.go' -and -not -name 'doc.go' -and -not -name '*.gen.go' -type f -exec sh -c 'mv "$$1" "$${1%.go}".gen.go' - '{}' \;

.PHONY: generate-k8s-client
generate-k8s-client:
	# generate kube api type wrappers for Aeraki types
